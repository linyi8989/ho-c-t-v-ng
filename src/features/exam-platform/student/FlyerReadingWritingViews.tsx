...
function TwoColumn({ imageUrl, optionalImage = false, children }: { imageUrl?: string; optionalImage?: boolean; children: ReactNode }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(0,44%)_minmax(0,56%)]"><div className="lg:sticky lg:top-4 lg:self-start">{imageUrl ? <ExamImageViewer src={imageUrl} alt="Ảnh bài Flyers Reading & Writing" maxHeight="min(70vh, 640px)" className="border border-slate-200 bg-white" /> : <div className="rounded-2xl border-2 border-dashed border-amber-300 bg-amber-50 p-10 text-center text-sm font-black text-amber-900">Chưa tải ảnh hiển thị.</div>}</div><div className="space-y-4">{children}</div></div>;
}
