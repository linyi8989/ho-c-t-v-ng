...
function PartTwo({ unit, answers, onAnswer }: { unit: ExamPartContent } & Omit<Props, 'part'>) {
  return <div className="space-y-3"><Example unit={unit} /><FlyerLetterMatchingView part={unit} answers={answers} onAnswer={onAnswer} /></div>;
}

function PartThree({ unit, answers, onAnswer }: { unit: ExamPartContent } & Omit<Props, 'part'>) {
  return <div className="mx-auto max-w-5xl space-y-4">{unit.passage?.trim() && <section className="rounded-2xl border-2 border-blue-300 bg-blue-50 p-4 text-sm font-black leading-7 text-slate-950" data-ket-listening-part3-prompt>{unit.passage}</section>}<Example unit={unit} /><div className="space-y-3" data-ket-listening-dialogue-choices>{unit.questions.map((question, index) => <fieldset key={question.id} className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
...
function FormPart({ unit, answers, onAnswer }: { unit: ExamPartContent } & Omit<Props, 'part'>) {
  return <div className="mx-auto max-w-5xl space-y-5">
    {unit.passage?.trim() && <section className="rounded-2xl border-2 border-blue-300 bg-blue-50 p-4 sm:p-5" data-ket-listening-text-source><p className="text-[10px] font-black uppercase tracking-wide text-blue-800">Instructions and example</p><div className="mt-3 whitespace-pre-wrap text-sm font-semibold leading-7 text-slate-950">{unit.passage}</div></section>}
