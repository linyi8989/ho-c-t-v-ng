...
function PartView({ part, answers, onAnswer, starterListening = false, starterReadingWriting = false }: { part: ExamPartContent; answers: ExamAnswers; onAnswer: (questionId: string, value: ExamAnswerValue) => void; starterListening?: boolean; starterReadingWriting?: boolean }) {
  ...
  return <div className="space-y-6">{units.map((unit, index) => {
    ...
    return <section key={unit.id} className={units.length > 1 ? 'rounded-2xl border border-slate-200 bg-slate-50/60 p-4 sm:p-5' : ''} data-exam-block={index + 1}>
      ...
      <div className="space-y-4">{unit.passage && <div className="rounded-2xl border border-slate-200 bg-white p-5 whitespace-pre-wrap text-sm font-semibold leading-7 text-slate-700">{unit.passage}</div>}{!special && unit.imageUrl && <ExamImageViewer src={unit.imageUrl} alt={`Ảnh Part ${unit.part}`} className="border border-slate-200 bg-white" />}{unit.audioUrl && !starterListening && <audio controls preload="metadata" src={unit.audioUrl} className="w-full" />}{special ? <StarterInteractionView part={unit} answers={answers} onAnswer={onAnswer} /> : unit.questions.map(question => <QuestionView key={question.id} question={question} value={questionAnswerValue(answers[question.id])} onChange={value => onAnswer(question.id, value)} />)}</div>
    </section>;
  })}</div>;
}
...
export default function GenericExamLearningArea({ moduleId, paperId, setId, accessToken = '', onBack }: Props) {
  ...
  if (moduleId === 'starter' && paperId === 'listening') return <main id="listening-exam-root" className="min-h-screen bg-gradient-to-b from-sky-300 via-sky-100 to-emerald-100 p-2 sm:p-4">
    ...
    <section className="mx-auto max-w-[1500px] rounded-[1.75rem] border-[10px] border-sky-700 bg-white p-3 shadow-2xl sm:p-6">
      ...
      <div className={activePart.part === 1 || activePart.part === 4 ? 'h-[calc(100vh-290px)] min-h-[400px] overflow-hidden p-1' : 'max-h-[calc(100vh-290px)] min-h-[400px] overflow-y-auto p-1'}><PartView part={activePart} answers={answers} starterListening onAnswer={(questionId, value) => setAnswers(previous => ({ ...previous, [questionId]: value }))} /></div>
    </section>
