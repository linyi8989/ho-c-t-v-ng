...
import { isFixedPetListeningContent } from '../petListeningMigration';
import { isFixedPetReadingContent } from '../petReadingMigration';
import { isFixedPetWritingContent } from '../petWritingMigration';
...
import PetListeningPartView from './PetListeningViews';
import PetReadingPartView from './PetReadingViews';
import PetReadingResult from './PetReadingResult';
...
const answerPresent = (value: ExamAnswerValue | undefined) => {
  ...
  if (value && typeof value === 'object' && 'optionId' in value && 'text' in value) return Boolean(String(value.optionId || '').trim() && String(value.text || '').trim());
...
function QuestionView({ question, value, onChange }: { key?: string; question: ExamQuestion; value?: ExamQuestionAnswerValue; onChange: (value: ExamQuestionAnswerValue) => void }) {
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" data-question-type={question.type}>
      ...
      {question.type === 'short-answer' && <input {...studentTypedAnswerGuards} value={Array.isArray(value) ? value.join(' ') : value || ''} onChange={event => onChange(event.target.value)} className="mt-4 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm font-bold outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100" placeholder={question.maxWords ? `Tối đa ${question.maxWords} từ` : 'Nhập câu trả lời'} />}
      {question.type === 'long-writing' && <div className="mt-4"><textarea {...studentTypedAnswerGuards} value={Array.isArray(value) ? value.join(' ') : value || ''} onChange={event => onChange(event.target.value)} className="min-h-64 w-full rounded-xl border border-slate-300 p-4 text-sm font-semibold leading-6 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100" placeholder="Viết bài của bạn tại đây…" /><p className={`mt-2 text-right text-xs font-black ${wordCount(String(value || '')) >= Number(question.minWords || 0) ? 'text-emerald-700' : 'text-amber-700'}`}>{wordCount(String(value || ''))}/{question.minWords || 0} từ tối thiểu</p>{question.rubric && <details className="mt-2 rounded-xl bg-slate-50 p-3 text-xs text-slate-600"><summary className="cursor-pointer font-black">Tiêu chí chấm</summary><p className="mt-2 whitespace-pre-wrap font-semibold leading-5">{question.rubric}</p></details>}</div>}
    </article>
  );
}

function PartView({ moduleId, paperId, part, answers, onAnswer, standaloneWriting = false, starterListening = false, flyerListening = false, ketListening = false, petListening = false, starterReadingWriting = false, flyerReadingWriting = false, ketReadingWriting = false, petReading = false, petWriting = false }: { moduleId: Exclude<ExamModuleId, 'mover'>; paperId: ExamPaperId; part: ExamPartContent; answers: ExamAnswers; onAnswer: (questionId: string, value: ExamAnswerValue) => void; standaloneWriting?: boolean; starterListening?: boolean; flyerListening?: boolean; ketListening?: boolean; petListening?: boolean; starterReadingWriting?: boolean; flyerReadingWriting?: boolean; ketReadingWriting?: boolean; petReading?: boolean; petWriting?: boolean }) {
  if (standaloneWriting) return <StandaloneWritingPartView part={part} answers={answers} onAnswer={onAnswer} />;
  ...
  if (petReading) return <PetReadingPartView part={part} answers={answers} onAnswer={onAnswer} />;
  if (petWriting) return <PetWritingPartView part={part} answers={answers} onAnswer={onAnswer} />;
  if (petListening) return <PetListeningPartView part={part} answers={answers} onAnswer={onAnswer} />;
...
export default function GenericExamLearningArea({ moduleId, paperId, setId, accessToken = '', onBack }: Props) {
  ...
  useEffect(() => {
    if (!(moduleId === 'writing' || (playable && isFixedPetWritingContent(playable.content))) || !result || result.status !== 'completed' || !playable?.content.showReviewAfterSubmit || review || reviewLoading || !reviewRunSecret) return;
  ...
  if (result && isFixedPetListeningContent(playable.content)) return <StarterListeningResult result={result} review={review} playable={playable} answers={answers} reviewLoading={reviewLoading} error={error} onReview={() => void loadReview()} onRetry={() => void start(true)} onBack={onBack} />;
  ...
  if (result && isFixedPetReadingContent(playable.content)) return <PetReadingResult result={result} review={review} playable={playable} reviewLoading={reviewLoading} error={error} onReview={() => void loadReview()} onRetry={() => void start(true)} onBack={onBack} />;
  ...
  if (result && isFixedPetWritingContent(playable.content)) return <PetWritingResult result={result} review={review} playable={playable} reviewLoading={reviewLoading} gradeRetrying={gradeRetrying} error={error} onReview={() => void loadReview()} onRetryGrade={() => void retryWritingGrade()} onRetry={() => void start(true)} onBack={onBack} />;
  ...
  const fixedPetWriting = isFixedPetWritingContent(playable.content);
  const hidePetWritingPartHeader = fixedPetWriting && activePart.part === 2;
  const ketListening = isFixedKetListeningContent(playable.content);
  const petListening = isFixedPetListeningContent(playable.content);
  const fixedListening = ketListening || petListening;
