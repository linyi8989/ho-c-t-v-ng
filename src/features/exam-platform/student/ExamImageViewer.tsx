...
export default function ExamImageViewer({
  ...
  return <>
    ...
    {open && <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={`Xem ảnh toàn màn hình: ${alt}`} onClick={event => event.stopPropagation()} className="exam-platform-image-dialog fixed inset-0 z-[1000] flex flex-col bg-slate-950/95 p-3 sm:p-5">
      ...
      <div className="min-h-0 flex-1 overflow-auto rounded-2xl bg-black/40 p-4">
        <div className="flex min-h-full min-w-full items-center justify-center">
          <img src={src} alt={alt} draggable={false} className="block h-auto max-w-none origin-center object-contain transition-transform" style={{ maxHeight: zoom <= 1 ? 'calc(100vh - 8rem)' : 'none', width: zoom <= 1 ? 'auto' : `${zoom * 100}%`, transform: zoom <= 1 ? `scale(${zoom})` : undefined }} />
        </div>
      </div>
    </div>}
  </>;
