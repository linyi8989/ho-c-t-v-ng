  ...
  return (
    <label className="block text-xs font-black text-slate-700">
      ...
      <div className="mt-1 flex flex-col gap-2 sm:flex-row">
        <select value={assetId || ''} onChange={event => onChange(active.find(asset => asset.id === event.target.value))} className={fieldClass}>
        ...
        <label className="inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-2.5 text-xs font-black text-indigo-700">
...
function QuestionEditor({
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-3"><AssetField token={token} kind="image" label="Ảnh riêng của câu (không bắt buộc)" assets={assets} assetId={question.imageAssetId} onUploaded={onAssets} onChange={asset => onChange({ ...question, imageAssetId: asset?.id, imageUrl: asset?.url })} /></div>
          ...
          {question.options.map((option, index) => {
            ...
            return (
              <div key={option.id} className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-2">
                ...
                <AssetField token={token} kind="image" label={`Ảnh lựa chọn ${option.label} (không bắt buộc)`} assets={assets} assetId={option.imageAssetId} onUploaded={onAssets} onChange={asset => updateOption(index, { imageAssetId: asset?.id, imageUrl: asset?.url })} />
              </div>
            );
          })}
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4">
        <p className="text-xs font-black uppercase tracking-wide text-indigo-700">{partDefinition.displayName}</p>
        <h3 className="mt-1 text-xl font-black text-slate-900">{partDefinition.title}</h3>
        <p className="mt-1 text-xs font-semibold text-slate-600">{partDefinition.questionCountFlexible ? `Phân bổ linh hoạt; toàn bài phải đủ ${definition.totalQuestionCount} câu.` : `${partDefinition.questionCount} câu cố định.`}</p>
      </div>
      ...
      <label className="block text-xs font-black text-slate-700">Đoạn đọc/nội dung chung của Part<textarea value={part.passage || ''} onChange={event => onChange({ ...part, passage: event.target.value })} className={`mt-1 min-h-32 ${fieldClass}`} /></label>
      <div className="grid gap-3 md:grid-cols-2">
        <AssetField token={token} kind="image" label="Ảnh chung của Part" assets={assets} assetId={part.imageAssetId} onUploaded={onAssets} onChange={asset => onChange({ ...part, imageAssetId: asset?.id, imageUrl: asset?.url })} />
        {partDefinition.requiresAudio && <AssetField token={token} kind="audio" label="Audio bắt buộc" assets={assets} assetId={part.audioAssetId} onUploaded={onAssets} onChange={asset => onChange({ ...part, audioAssetId: asset?.id, audioUrl: asset?.url })} />}
      </div>

      <details className="rounded-2xl border border-violet-200 bg-violet-50 p-4">
        <summary className="cursor-pointer text-sm font-black text-violet-800"><FileJson size={16} className="mr-2 inline" />Smart Import bằng JSON đã trích xuất</summary>
        <p className="mt-2 text-xs font-semibold leading-5 text-violet-700">Dán object JSON của riêng Part này. Không đưa ID hoặc URL media vào JSON. Với câu chọn đáp án, dùng <code>correctOptionLabels</code> hoặc <code>correctOptionIndexes</code> (đánh số từ 1); nếu không có ảnh đáp án chính thức, để trống và giáo viên chọn trong form.</p>
        <textarea value={importJson} onChange={event => setImportJson(event.target.value)} className={`mt-3 min-h-36 font-mono ${fieldClass}`} placeholder='{"title":"...","questions":[{"type":"single-choice","prompt":"...","options":[{"label":"A","text":"..."}],"correctOptionLabels":["A"]}]}' />
        <button type="button" onClick={async () => {
          try {
            const parsed = JSON.parse(importJson);
            const candidate = parsed.part && typeof parsed.part === 'object' ? parsed.part : parsed;
            const result = await examPlatformApi.validateSmartImport(token, content.moduleId, content.paperId, partIndex, candidate, part);
            onChange(result.part);
            onMessage({ text: `Đã nhập và kiểm tra ${partDefinition.displayName}.${result.warnings?.length ? ` ${result.warnings.length} mục cần giáo viên xác nhận.` : ''} Hãy rà soát đáp án trước khi xuất bản.` });
          } catch (error: any) { onMessage({ text: error.message || 'JSON Smart Import không hợp lệ.', error: true }); }
        }} className="mt-3 inline-flex items-center gap-2 rounded-xl bg-violet-700 px-4 py-2.5 text-xs font-black text-white"><FileJson size={15} />Kiểm tra và nhập vào bản nháp</button>
      </details>

      <div className="space-y-3">
        {part.questions.map((question, index) => <QuestionEditor key={question.id} question={question} partNumber={part.part} allowedTypes={partDefinition.allowedQuestionTypes} token={token} assets={assets} onAssets={onAssets} onChange={value => updateQuestion(index, value)} />)}
      </div>
      {partDefinition.questionCountFlexible && (
...
function PaperAdmin({ token, moduleId, paperId, onBack }: PaperAdminProps) {
  ...
  const startNew = () => {
    setEditingId(''); setContent(createDefaultExamContent(definition)); setVisibility('draft'); setRevision(0); setDirty(true); setValidationErrors([]); setStep(0); setAutosave('idle'); setResults(null);
  };
  const edit = async (setId: string) => {
    setBusy(true);
    try {
      const set = await examPlatformApi.getAdminSet(token, moduleId, paperId, setId);
      setEditingId(set.id); setContent(set.draftContent); setVisibility(set.visibility || 'draft'); setRevision(Number(set.draftRevision || 0)); setDirty(false); setValidationErrors(set.validationErrors || []); setStep(0); setAutosave('saved');
  ...
  const steps = ['Thông tin', ...definition.parts.map(part => part.displayName), 'Preview'];
  if (editing) return (
    <div className="space-y-4" id="generic-exam-editor" data-module={moduleId} data-paper={paperId}>
      <header className="sticky top-0 z-20 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-sm backdrop-blur">
        ...
        <nav className="mt-3 flex gap-2 overflow-x-auto pb-1">{steps.map((label, index) => <button key={label} type="button" onClick={() => setStep(index)} data-active={step === index} className={`shrink-0 rounded-xl px-3 py-2 text-xs font-black ${step === index ? 'bg-indigo-600 text-white' : 'border border-slate-200 bg-white text-slate-600'}`}>{label}</button>)}</nav>
      </header>
      ...
      <section className="rounded-3xl border border-slate-200 bg-slate-50 p-4 sm:p-6">
        {step === 0 && <div className="space-y-4"><div className="grid gap-3 md:grid-cols-2"><label className="text-xs font-black text-slate-700">Tên bộ đề<input value={content.title} onChange={event => updateContent({ ...content, title: event.target.value })} className={`mt-1 ${fieldClass}`} /></label><label className="text-xs font-black text-slate-700">Cấp độ<input value={content.level} onChange={event => updateContent({ ...content, level: event.target.value })} className={`mt-1 ${fieldClass}`} /></label><label className="text-xs font-black text-slate-700 md:col-span-2">Mô tả<textarea value={content.description} onChange={event => updateContent({ ...content, description: event.target.value })} className={`mt-1 min-h-24 ${fieldClass}`} /></label><label className="text-xs font-black text-slate-700">Thời gian (phút)<input type="number" min={0} value={content.timeLimitMinutes || 0} onChange={event => updateContent({ ...content, timeLimitMinutes: Number(event.target.value) || undefined })} className={`mt-1 ${fieldClass}`} /></label><label className="text-xs font-black text-slate-700">Hiển thị<select value={visibility} onChange={event => { setVisibility(event.target.value as ExamVisibility); setDirty(true); }} className={`mt-1 ${fieldClass}`}><option value="draft">Bản nháp</option><option value="public">Công khai</option><option value="assignment">Theo link/bài giao</option></select></label></div><AssetField token={token} kind="image" label="Ảnh bìa" assets={assets} assetId={content.coverAssetId} onUploaded={asset => setAssets(previous => [asset, ...previous.filter(item => item.id !== asset.id)])} onChange={asset => updateContent({ ...content, coverAssetId: asset?.id, coverUrl: asset?.url })} /><label className="flex items-center gap-2 text-sm font-black text-slate-700"><input type="checkbox" checked={content.showReviewAfterSubmit} onChange={event => updateContent({ ...content, showReviewAfterSubmit: event.target.checked })} />Cho học sinh xem đáp án sau khi hoàn thành và được chấm xong</label></div>}
        {step > 0 && step <= definition.parts.length && <PartEditor token={token} content={content} partIndex={step - 1} assets={assets} onAssets={asset => setAssets(previous => [asset, ...previous.filter(item => item.id !== asset.id)])} onChange={part => updateContent({ ...content, parts: content.parts.map((item, index) => index === step - 1 ? part : item) })} onMessage={setMessage} />}
        {step === definition.parts.length + 1 && <div className="space-y-4"><div className="rounded-2xl bg-white p-5"><p className="text-xs font-black uppercase text-indigo-600">Preview cấu trúc</p><h3 className="mt-1 text-2xl font-black text-slate-900">{content.title}</h3><p className="mt-2 text-sm font-semibold text-slate-500">{content.description}</p><p className="mt-3 text-sm font-black text-indigo-700">{content.parts.length} Part/Section · {totalQuestions} câu/task · {content.timeLimitMinutes || 'Không giới hạn'} phút</p></div>{content.parts.map(part => <article key={part.id} className="rounded-2xl border border-slate-200 bg-white p-4"><h4 className="font-black text-slate-900">Part {part.part}: {part.title}</h4>{part.passage && <p className="mt-2 whitespace-pre-wrap text-sm text-slate-600">{part.passage}</p>}{part.imageUrl && <img src={part.imageUrl} alt="" className="mt-3 max-h-72 rounded-xl object-contain" />}{part.audioUrl && <audio controls src={part.audioUrl} className="mt-3 w-full" /> }<ol className="mt-3 list-decimal space-y-2 pl-5 text-sm font-semibold text-slate-700">{part.questions.map(question => <li key={question.id}>{question.prompt} <span className="text-xs text-slate-400">({question.type})</span></li>)}</ol></article>)}</div>}
      </section>
    </div>
  );
