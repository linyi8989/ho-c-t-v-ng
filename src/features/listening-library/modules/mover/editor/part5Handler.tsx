...
function ScenePart5Editor({ part, props }: { part: ListeningPart5SceneColourDraw; props: MoverPartEditorProps<ListeningPart5> }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="space-y-3 rounded-2xl border border-rose-200 bg-rose-50/30 p-4"><div><h4 className="text-sm font-black text-rose-950">Vị trí Draw do GPT-5.6 Sol nhận diện</h4><p className="text-[11px] font-semibold text-rose-800">Sol chuyển vị trí từ ảnh đáp án sang đúng hệ tọa độ ảnh đề. Vùng này chỉ dùng cho staff/backend và không gửi xuống student.</p></div>{placeActions.length > 0 ? <><div className="relative mx-auto max-w-4xl overflow-hidden rounded-xl border bg-white"><img src={assetUrl(part.sceneAssetId)} alt="Xem trước vị trí Draw" className="block h-auto w-full" />{placeActions.map(item => <div key={item.id} style={previewRegionStyle(item.region)} className="absolute flex items-center justify-center border-2 border-rose-600 bg-rose-300/30 text-[10px] font-black text-rose-950"><span className="rounded bg-white/90 px-1">{item.label}</span></div>)}</div><details className="rounded-xl border border-rose-200 bg-white p-3"><summary className="cursor-pointer text-xs font-black text-rose-800">Chỉnh nâng cao nếu Sol đặt sai vị trí</summary><div className="mt-3"><ListeningRegionEditor imageUrl={assetUrl(part.sceneAssetId)} items={placeActions} onChange={items => commit({ ...part, questions: part.questions.map(question => ({ ...question, actions: question.actions.map(action => { if (action.type !== 'place_object') return action; const next = items.find(item => item.id === action.id)?.region; return next ? { ...action, targetRegion: next, geometryConfirmedByTeacher: JSON.stringify(next) !== JSON.stringify(action.targetRegion) ? true : action.geometryConfirmedByTeacher } : action; }) })) })} /></div></details></> : <div className="rounded-xl border-2 border-dashed border-rose-200 bg-white px-4 py-8 text-center text-xs font-bold text-rose-700">Chưa có vị trí Draw. Chọn đủ Ảnh đề bài, Ảnh đáp án và Ảnh đáp án + vị trí rồi chạy GPT-5.6 Sol.</div>}</section>
  </div>;
}
...
export default function MoverPart5Editor(props: MoverPartEditorProps<ListeningPart5>) {
  ...
  const importAnalysis = (candidate: ListeningSmartImportCandidate) => {
    if (candidate.data.part !== 5) throw new Error('Dữ liệu AI không đúng Part 5.');
  ...
  return <div className="space-y-5">
    ...
    <SmartImportPanel initialPreferredProvider="stali:gpt-5.6-sol" token={token} part={part} assets={assets} capability={smartImportCapability} onCandidateChange={onImportCandidateChange} onAnalyzed={importAnalysis} onUpload={onUpload} analyzeLabel="GPT-5.6 Sol đọc 3 ảnh và nhập Part 5" analyzedNotice="Đã nhập nội dung, màu và gợi ý vị trí Draw vào bài soạn. Hãy tải icon và xác nhận vùng Colour/Draw trực tiếp trong bảng." />
