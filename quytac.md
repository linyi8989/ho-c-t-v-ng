...
- Transcript Mover là plain text tùy chọn theo từng Part, tối đa 20.000 ký tự và
  chỉ lưu trong working draft/immutable published version. Phải loại transcript
  khỏi playable, prepare, submit-summary và API công khai; chỉ dựng từ đúng
  `versionId` cho endpoint review/History đã kiểm tra owner, guest run secret và
  `showReviewAfterSubmit`. Không sao chép transcript vào từng attempt/detail.
- Kết quả trực quan sau khi nộp và chi tiết Listening trong Learning History phải dùng cùng một snapshot presentation có version. Snapshot chỉ phủ trạng thái lên chính asset ảnh của immutable version, không tạo hoặc ghi lại ảnh mới. Mặc định đúng dùng tích xanh; sai dùng dấu X đỏ và hiện đáp án đúng ngay dưới đáp án học sinh; bỏ trống có trạng thái riêng. Riêng Part 3 dạng nối hình không phủ icon/nhãn lên ảnh: đường học sinh nối đúng giữ màu xanh dương, đường sai chuyển đỏ và đường đúng được vẽ thêm bằng nét xanh lá gạch đứt. Các đường Part 3 phải được phân tuyến qua ba làn riêng ở mỗi hành lang trái/phải; đường sai và đường đúng của cùng câu không dùng chung làn khi còn lựa chọn, và khi tái sử dụng làn phải có offset nhỏ để tránh chồng nét. Example không chấm điểm không được tạo review item. Part 5 phải resolve submission theo cả `actionId` và khóa tương tác tự nhiên (`objectId`/`paletteItemId` + vùng Draw); đã đặt lựa chọn sai trong vùng thì là `incorrect`, không được biến thành `unanswered`. Tọa độ Draw tiếp tục được giữ để chấm nhưng không hiện trong câu trả lời học sinh, và thẻ review Part 5 không lặp dòng đáp án đúng vì prompt đã chứa yêu cầu màu/vật. Snapshot không được chứa ID kỹ thuật hoặc private `targetRegion`; review Draw chỉ được dùng điểm hiển thị an toàn suy ra ở backend. Nếu `showReviewAfterSubmit` không cho xem thì phải loại toàn bộ snapshot, không chỉ xóa từng trường đáp án.
- Điều hướng kết quả Listening phải luôn có tương phản rõ: Part đang chọn dùng nền xanh đậm/chữ trắng/vòng selected, Part chưa chọn dùng nền xanh nhạt/chữ xanh đậm. Nút chuyển Part trái/phải phải có gutter riêng bên ngoài nội dung Part, không phủ ảnh hoặc thẻ đáp án; icon và vùng bấm phải đủ lớn, còn nút khóa ở biên vẫn phải nhìn thấy.
...
- Smart Import Parts 1-5 tạm thời mặc định chọn `Thông số bên ngoài`: mỗi Part chỉ nhận một `Ảnh đề bài` và JSON versioned `mover-partN-external-v1`, không gọi provider AI, rồi parse/normalize/validate và ghép qua direct-import hiện có. JSON không được chứa ID kỹ thuật. Part 2 vẫn mở crop tranh minh họa; Part 3 chỉ dùng geometry hợp lệ hoặc giữ geometry draft tương ứng; Part 4 vẫn dò/crop/upload 15/18 hình và khi không nhận đủ khung phải giữ nguyên ảnh lựa chọn để giáo viên sửa; Part 5 vẫn yêu cầu giáo viên vẽ Colour mask, tải icon và xác nhận Draw region. Dropdown nguồn xử lý chỉ được có đúng ba lựa chọn theo thứ tự: `Thông số bên ngoài`, `Stali · ChatGPT 5.6 Sol`, `DevQuota · ChatGPT 5.6 Sol`. Chọn provider nào phải gọi đúng provider đó, không fallback âm thầm sang Gemini, OpenAI trực tiếp hoặc model khác. Giáo viên luôn được đổi nguồn xử lý thủ công cho từng lượt.
- Mỗi form `Thông số bên ngoài` Part 1-5 phải có nút `Sao chép hướng dẫn cho AI`. Nội dung clipboard phải được sinh từ cùng help + JSON template versioned mà parser hiện dùng, gồm quy tắc Part, cấm ID kỹ thuật/audio/transcript và yêu cầu chỉ trả JSON không Markdown. Sao chép không được thay textarea, candidate hoặc draft; phải có trạng thái `Đã sao chép` và fallback sao chép thủ công khi Clipboard API không dùng được.
